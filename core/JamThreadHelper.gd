...
var _product_mtx: Mutex = Mutex.new()
...
func producer_wrapper(id: int, producer: Callable) -> void:
	var r: Variant = producer.call()
	put_product(id, ThreadProduct.make(r))


func _add_a_producer(producer: Callable) -> ProducerHandle:
	var product_id: int = randi()
	while product_id in _product_map:
		product_id = randi()
	var task_id: int = WorkerThreadPool.add_task(producer_wrapper.bind(product_id, producer))
...
func run_threaded_producer(producer: Callable) -> ThreadProduct:
	var handle: ProducerHandle = _add_a_producer(producer)
	while true:
...
class ProducerHandle:
	...
	func _init(prod_id: int, tsk_id: int) -> void:
...
class ThreadProduct:
	...
	static func make(val: Variant) -> ThreadProduct:
		var p: ThreadProduct = ThreadProduct.new()
		p.value = val
		return p
