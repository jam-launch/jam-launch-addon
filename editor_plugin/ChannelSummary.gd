...
func set_channel(channel_data: Dictionary, release_name: String = "") -> void:
	c = channel_data
	...
	title.push_bold()
	title.add_text(channel_data.get("name") as String)
	title.pop_all()
	title.push_context()
...
func _on_config_change(_toggled_on: bool) -> void:
	if not c.get("name"):
		return
